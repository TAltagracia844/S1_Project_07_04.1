...
function verifyAcres() {
    ...
    try {
        ...
        messageHeadElement.innerHTML = ""; // remove anymore former recommendation heading
        testFormCompleteness();
    }
}
...
function verifyCrops() {
    ...
    // remove any former recommendation heading messageElement.innerHTML = message; 
...
function verifyMonths() {
    ...
    validity = false;
    messageText = message;
    monthsBox.value = ""; // remove errorous entry form input box
} finally {
...
function createRecommendation() {
    if (acresBox.value <= 5000) { // 5000 acres or less, no crop test needed
        ...
        if (monthsBox.value <= 9) { // 9 or fewer months per year, no crop test needed
            ...
            if (document.getElementById("wheat").checked || document.getElementById("corn").checked ||
